...
// endpoint, which forwards over Yggdrasil to the remote peer's MCP router
// and on to the registered server.
//
// Per OTel MCP semantic conventions, W3C `traceparent` and `baggage` ride
// in `params._meta`; we inject them from the active OTel context so the
// remote handler can extract and continue the trace + originator baggage.
